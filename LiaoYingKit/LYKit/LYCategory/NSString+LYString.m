...
#pragma mark - 正则相关
- (BOOL)ly_isValidateByRegex:(NSString *)regex
{
...
- (BOOL)ly_isMobileNumberClassification{
    ...
    if (([self    ly_isValidateByRegex:CM])
        || ([self ly_isValidateByRegex:CU])
        || ([self ly_isValidateByRegex:CT])
        || ([self ly_isValidateByRegex:PHS]))
    {
        return YES;
    }
    else
...
- (BOOL)ly_isEmailAddress{
    ...
    return [self ly_isValidateByRegex:emailRegex];
}
...
- (BOOL)ly_simpleVerifyIdentityCardNum
{
    ...
    return [self ly_isValidateByRegex:regex2];
}
...
- (BOOL)ly_isCarNumber{
    ...
    return [self ly_isValidateByRegex:carRegex];
}

- (BOOL)ly_isMacAddress{
    NSString * macAddRegex = @"([A-Fa-f\\d]{2}:){5}[A-Fa-f\\d]{2}";
    return  [self ly_isValidateByRegex:macAddRegex];
}

- (BOOL)ly_isValidUrl
{
    NSString *regex = @"^((http)|(https))+:[^\\s]+\\.[^\\s]*$";
    return [self ly_isValidateByRegex:regex];
}

- (BOOL)ly_isValidChinese;
{
    NSString *chineseRegex = @"^[\u4e00-\u9fa5]+$";
    return [self ly_isValidateByRegex:chineseRegex];
}

- (BOOL)ly_isValidPostalcode {
    NSString *postalRegex = @"^[0-8]\\d{5}(?!\\d)$";
    return [self ly_isValidateByRegex:postalRegex];
}

- (BOOL)ly_isValidTaxNo
{
    NSString *taxNoRegex = @"[0-9]\\d{13}([0-9]|X)$";
    return [self ly_isValidateByRegex:taxNoRegex];
}
...
- (NSString *)ly_stringByRemovingScriptsAndStrippingHTML
{
    ...
    return [mString ly_stringByStrippingHTML];
}
...
- (NSString *)ly_to16MD5 {
    ...
    return [[self ly_toMD5] substringWithRange:NSMakeRange(8, 16)];
}
