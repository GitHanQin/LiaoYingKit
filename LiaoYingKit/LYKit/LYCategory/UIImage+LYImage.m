...
- (UIImage *)ly_blur
{
    return [self ly_imgWithLightAlpha:0.1 radius:3 colorSaturationFactor:1];
}

- (UIImage *)ly_imgWithLightAlpha:(CGFloat)alpha radius:(CGFloat)radius colorSaturationFactor:(CGFloat)colorSaturationFactor
{
    UIColor *tintColor = [UIColor colorWithWhite:1.0 alpha:alpha];
    return [self ly_imgBluredWithRadius:radius tintColor:tintColor saturationDeltaFactor:colorSaturationFactor maskImage:nil];
}

- (UIImage *)ly_imgBluredWithRadius:(CGFloat)blurRadius tintColor:(UIColor *)tintColor saturationDeltaFactor:(CGFloat)saturationDeltaFactor maskImage:(UIImage *)maskImage
{
...
+ (UIImage *)ly_animatedImageWithAnimatedGIFData:(NSData *)data {
    return animatedImageWithAnimatedGIFReleasingImageSource(CGImageSourceCreateWithData(toCF data, NULL));
}

+ (UIImage *)ly_animatedImageWithAnimatedGIFURL:(NSURL *)url {
...
- (UIImage *)ly_imageScaledToFitSize:(CGSize)size
{
    ...
    {
        return [self ly_imageScaledToSize:CGSizeMake(size.width, size.width / aspect)];
    }
    else
    {
        return [self ly_imageScaledToSize:CGSizeMake(size.height * aspect, size.height)];
    }
}
...
- (UIImage *)ly_reflectedImageWithScale:(CGFloat)scale
{
    ...
    CGContextClipToMask(context, bounds, [[self class] ly_gradientMask]);
...
- (UIImage *)ly_imageWithReflectionWithScale:(CGFloat)scale gap:(CGFloat)gap alpha:(CGFloat)alpha
{
    ...
    UIImage *reflection = [self ly_reflectedImageWithScale:scale];
...
{
    return [self ly_rotate:UIImageOrientationDownMirrored];
}

/** 水平翻转 */
- (UIImage *)ly_flipHorizontal
{
    return [self ly_rotate:UIImageOrientationUpMirrored];
}
...
{
    return [self ly_imageRotatedByRadians:kDegreesToRadian(degrees)];
}
...
- (UIImage *)ly_rescaleImageToPX:(CGFloat )toPX
{
    ...
    return [self ly_rescaleImageToSize:size];
}
