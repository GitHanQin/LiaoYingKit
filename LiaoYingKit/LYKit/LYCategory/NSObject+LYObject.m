...
-(void)ly_observeValueForKeyPath:(NSString *)keyPath
                     ofObject:(id)object
...
-(void)ly_addObserverForKeyPath:(NSString *)keyPath
                   ...
                   withBlock:(KVOBlock)block
{
    [self ly_addObserver:self forKeyPath:keyPath options:options context:context withBlock:block];
}

-(void)ly_removeBlockObserverForKeyPath:(NSString *)keyPath
{
    [self ly_removeBlockObserver:self forKeyPath:keyPath];
}
...
-(NSArray*)ly_methodListInfo
{
    ...
    for (int i = 0; i < count ; i++)
    {
        ...
        for (int index=0; index<argumentsCount; index++) {
            ...
            [arguments addObject:[[self class] ly_decodeType:arg]];
        }
        
        NSString *returnTypeString =[[self class] ly_decodeType:returnType];
        NSString *encodeString = [[self class] ly_decodeType:encoding];
...
+ (NSArray *)ly_instanceVariable
{
    ...
    NSMutableArray *result = [NSMutableArray array];
    for (int i = 0; i < outCount; i++) {
        NSString *type = [[self class] ly_decodeType:ivar_getTypeEncoding(ivars[i])];
...
#pragma mark -- help
+ (NSDictionary *)ly_dictionaryWithProperty:(objc_property_t)property
{
...
+ (NSString *)ly_decodeType:(const char *)cString
{
    ...
    } else
    {
        if ([[result substringToIndex:1] isEqualToString:@"^"]) {
            result = [NSString stringWithFormat:@"%@ *",
                      [NSString ly_decodeType:[[result substringFromIndex:1] cStringUsingEncoding:NSUTF8StringEncoding]]];
        }
    }
    return result;
}
...
-(NSArray<NSString *> *)ly_getPropertyNameList{
...
-(void)ly_modelWithDictionary:(NSDictionary *)dict{
    //得到本模型的所有属性
    NSArray *propertyNameList=[self ly_getPropertyNameList];
    //遍历字典中的所有属性
